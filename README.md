...
| 4. 알림 | 로그인 → 본인 이메일 인증 → 시간 선택 | 로그인하지 않은 사용자는 날씨를 둘러볼 수 있지만, 구독 버튼은 로그인으로 자연스럽게 이어집니다. |
...
- 구독은 `Coders/Google 로그인`과 이메일 소유권 인증을 모두 통과해야 하며, 인증되지 않은 주소로는 전송하지 않습니다.
- API 오류는 `Failed to fetch` 같은 브라우저 문구 대신 사용자가 다음에 할 행동(재시도·위치 재선택·로그인)을 안내합니다.
- 모바일은 390px 폭에서도 가로 스크롤 없이 핵심 카드와 버튼을 사용할 수 있고, 데스크톱에서는 읽기 폭을 제한해 시선 이동을 줄입니다.
...
| 날씨 이메일 | 사용자 본인 이메일 1개, 15분 이메일 소유권 인증, 사용자 지정 시각(분 단위) 예약 발송, 스마트 위험 알림 |
| 구독 관리 | Google 로그인 계정(UUID) 귀속, 인증 이메일 1개만 연결, 알림 시각 변경, 해지, 개인정보 완전 삭제 |
| 웹 경험 | 토스형 반응형 UI, 메일과 동일한 결정 카드, 다크 모드, PWA 설치, 오프라인 상태 안내 |
...
- 구독 전 15분 유효 이메일 인증 챌린지를 발급하고, 인증 링크를 누른 주소 1개만 해당 `ownerId`에 일회성으로 연결합니다.
- 구독 요청의 이메일과 인증 챌린지의 이메일이 다르면 서버에서 거부하며, 인증 토큰은 해시로만 저장하고 사용 즉시 폐기합니다.
...
| `GET` | `/api/users/email-verification/confirm` | 인증 링크 확인 후 구독 화면으로 복귀 |
...
`/api/users/subscribe`는 운영 환경에서 `X-Coders-User`와 이메일 인증 토큰(`verificationToken`)을 모두 요구하며, 인증된 이메일 하나만 받습니다. `/api/users/me/notifications`의 알림 시각 필드는 `HH:mm` 형식의 `morningTime`, `afternoonTime`, `eveningTime`입니다. 운영 스케줄러는 매분 해당 시각의 사용자만 조회하고, 한국 표준시(`Asia/Seoul`)로 발송합니다.
...
- 현재 계정 인증은 운영 플랫폼의 Google 로그인을 사용합니다. 플랫폼은 앱에 원문 이메일 대신 프로젝트별 UUID만 전달하므로, 첫 구독에는 15분 이메일 소유권 인증이 필요합니다. Naver·Kakao 로그인은 공급자가 신뢰 가능한 이메일 claim을 제공하는지 확인한 뒤 추가할 예정입니다.
