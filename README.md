...
- 모바일은 390px 폭에서도 가로 스크롤 없이 사용할 수 있으며, 하단 고정 메뉴에서 `검색·오늘·구독`으로 바로 이동합니다. 데스크톱에서는 읽기 폭을 제한해 시선 이동을 줄입니다.
...
| 웹 경험 | 토스형 반응형 UI, 모바일 하단 빠른 메뉴, 메일과 동일한 결정 카드, 다크 모드, PWA 설치, 오프라인 상태 안내 |
