...
첫 화면에서 위치를 검색하면 아침·점심·저녁 날씨와 외출 준비를 먼저 확인할 수 있습니다. 체감 성향과 오늘 일정은 즉시 코디에 반영되며, 로그인한 사용자는 기존 구독 위치가 자동으로 열립니다.
...
2. **오늘·내일 비교 브리핑** — 현재 위치의 기온·강수확률·습도·풍속·외출 점수와 준비물을 날짜별로 비교합니다.
3. **여러 메일함 동시 구독** — 쉼표 또는 줄바꿈으로 최대 10개 이메일 주소를 한 번에 등록할 수 있습니다.
4. **동적 스타일링** — 기온, 비·눈, 바람, 대기질, 추위·더위 민감도와 일정 유형에 맞춰 상의·하의·아우터·신발·주의사항이 달라집니다.
...
- 날씨별 상세 조언
  ...
  - 눈 오는 날 미끄럼/보온 안내
- 날씨·기온·강수·바람·대기질·체감 성향·일정 유형을 조합한 동적 스타일링 추천
- 로그인 사용자의 기존 구독 위치와 개인화 설정을 첫 화면에 자동 복원
...
```http
GET /api/weather/daily?nx=61&ny=125&locationName=강남역&dayOffset=1&temperatureSensitivity=HEAT&activityType=OUTDOOR
```
...
서비스 화면의 `받는 이메일` 입력란에 기존 이메일을 입력하고 `이메일로 구독 취소`를 누르면 해당 주소의 알림이 중단됩니다. 쉼표 또는 줄바꿈으로 최대 10개까지 한 번에 처리할 수 있습니다.
...
PATCH /api/users/me/smart-alerts
...
스마트 알림은 강수·극단 기온·나쁜 공기질·강풍 중 사용자가 켠 조건만 평가합니다. 같은 날짜와 시간대에 같은 위험으로 메일을 이미 보냈다면 중복 전송하지 않습니다.
...
이미 구독한 이메일의 체감 성향과 일정 유형을 변경합니다. 이전 클라이언트 호환을 위해 연령대·성별 필드도 계속 받을 수 있지만, 새 UI는 체감과 실제 일정 중심으로 추천합니다.
...
```text
temperatureSensitivity: NONE, COLD, HEAT
activityType: DAILY, COMMUTE, OUTDOOR, FORMAL
```
