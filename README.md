...
현재 단계에서는 결제창이나 유료 배지를 붙이지 않습니다. 먼저 사용자가 날씨한편의 판단을 반복해서 쓰는지 검증하고, 그 다음에 비용을 지불할 만큼 분명한 가치를 가진 기능만 상품화합니다.

| 단계 | 현재 제품에서 만드는 행동 | 확인할 지표 | 다음 판단 |
| --- | --- | --- | --- |
| 가치 경험 | 장소를 고르고 현재 날씨·외출 추천 시간을 확인 | 장소 검색 성공 | 추천 정확도와 첫 화면 개선 |
| 활성화 | 추천 외출 시간의 `30분 전에 매일 받기`를 선택 | 추천 시간 기반 구독 완료 | 어떤 추천이 구독으로 이어지는지 확인 |
| 습관 | 정해둔 시간에 개인화 브리핑을 받고 다시 방문 | 구독 유지·해지, 발송 성공률 | 반복 사용이 약하면 유료화보다 알림 품질 개선 |
| 수익화 준비 | 충분한 유지율이 확인된 사용자군에 고급 가치를 검증 | 코호트 유지율·유료 의향 | 장기 여행, 가족 위치 묶음, 전문 위험 알림 중 수요가 확인된 항목만 실험 |

이번 버전은 `날씨 확인 → 추천 시간 발견 → 알림 시간 자동 설정 → 이메일 인증 → 반복 수신`을 하나의 흐름으로 연결했습니다. 기존 구독자는 현재 알림을 지우지 않고 추천 시간대만 추가할 수 있습니다. 운영 지표 `weather.product.funnel`은 고정된 단계·유입 태그만 기록하며 이메일, 계정 ID, 검색어, 좌표는 수집하지 않습니다.
...
- 첫 화면의 라이브 날씨 카드는 선택한 지역의 실제 기온·체감온도·강수·바람과 가까운 4개 시간의 기온·강수확률을 즉시 보여주며, 맑음·흐림·비·눈에 따라 하늘 색과 그래픽이 자동으로 달라집니다.
...
| 외출 타이밍 레이더 | 시간별 강수·바람·기온을 비교해 가장 부담이 적은 연속 외출 시간창과 점수를 추천하고, 한 번의 선택으로 30분 전 반복 알림을 준비 |
...
메일 본문은 모바일 수신을 기준으로 구성하며, 이미지와 같은 파란색 히어로 카드·핵심 지표·외출 판단에 더해 현재 시점부터 4개 시간대의 기온·하늘 상태·강수확률을 함께 표시합니다. 시간별 예보 원천자료를 불러오지 못하는 경우에도 기존 날씨 브리핑은 정상 발송하고 해당 보조 영역만 생략합니다.
...
- `360×800`, `390×844`, `768×1024`, `1440×900`에서 가로 넘침·잘림·고정 메뉴 겹침 확인
- 지역 검색, 날씨 로딩, 추천 시간 알림 설정, 이메일 인증, 성공·오류 상태 확인
- 키보드 초점, 폼 레이블, 44px 이상 모바일 터치 영역, 한국어 줄바꿈 확인
- 브라우저 콘솔 오류와 실패한 애플리케이션 요청 확인
...
| 이름 | 용도 | 필수 | 발급·준비 위치 |
| --- | --- | --- | --- |
| `WEATHER_API_KEY` | 기상청 예보·실황 | 운영 필수 | 공공데이터포털 기상청 단기예보 조회서비스 |
| `AIR_QUALITY_API_KEY` | 에어코리아 PM10·PM2.5 | 선택 | 공공데이터포털 한국환경공단 대기오염정보 |
| `KAKAO_REST_API_KEY` | 동네·역·건물 검색 | 운영 권장 | Kakao Developers 애플리케이션 REST API 키 |
| `WEATHER_WARNING_API_KEY` | 기상특보 | 선택 | 공공데이터포털 기상청 특보 API |
| `LIVING_WEATHER_API_KEY` | 자외선 등 생활기상 | 선택 | 공공데이터포털 생활기상지수 API |
| `POLLEN_API_KEY` | 꽃가루 위험 | 선택 | 공공데이터포털 꽃가루농도위험지수 API |
| `KAKAO_MOBILITY_REST_API_KEY` | 이동 경로 보조 | 선택 | Kakao Developers Mobility 권한 |
| `SMTP_HOST`, `SMTP_PORT`, `SMTP_USERNAME`, `SMTP_PASSWORD` | 인증번호·날씨 메일 발송 | 구독 기능 필수 | 사용하는 SMTP 공급자, Gmail은 앱 비밀번호 |
| `DB_URL`, `DB_USERNAME`, `DB_PASSWORD` | 운영 구독·발송 이력 저장 | 운영 필수 | PostgreSQL/MySQL 인스턴스 |
| `REDIS_URL` | 다중 인스턴스 요청 제한 | 운영 권장 | Redis 인스턴스 |
| `ADMIN_API_KEY` | Prometheus·metrics 보호 | 운영 필수 | 직접 생성한 긴 무작위 값 |
| `APP_BASE_URL` | 메일과 리다이렉트 기준 URL | 운영 필수 | 배포 도메인 |

### 데이터베이스

로컬 기본 프로필은 H2를 자동 사용합니다. 운영 프로필에서는 `DB_URL`·`DB_USERNAME`·`DB_PASSWORD`를 설정하면 Flyway가 `src/main/resources/db/migration`을 순서대로 적용합니다. Coders.kr 구성은 PostgreSQL을 사용하며, 임의로 운영 테이블을 초기화하지 않습니다.

## 저장소와 배포

- 원본 저장소: [boclair98/Weather](https://github.com/boclair98/Weather)
- 조직 포크: [coders-kr/Weather](https://github.com/coders-kr/Weather)
- 배포 소스: 원본 저장소의 실제 기본 브랜치
- 운영 서비스: [weather.coders.kr](https://weather.coders.kr)

업데이트는 로컬 테스트와 비밀정보 검사를 통과한 커밋을 원본 저장소에 먼저 푸시하고, 실제 GitHub 포크인 조직 저장소를 `gh repo sync coders-kr/Weather -b main`으로 동기화한 다음 원본 저장소에서 Coders.kr 재배포를 수행합니다. 배포 후에는 health와 실제 화면을 모바일·태블릿·데스크톱에서 다시 확인합니다. Coders.kr API 형식은 변경될 수 있으므로 배포 직전에 `https://coders.kr/llms.txt`의 최신 지침을 확인합니다.
...
- 현재 결제·유료 요금제·메일 열람 추적은 구현하지 않았습니다. 개인정보를 늘려 수집하지 않고 반복 사용과 구독 유지가 검증된 뒤 수익화 실험을 진행합니다.
- 별도 라이선스 파일이 없어 재사용 권한은 명시적으로 부여되지 않았습니다. 사용·배포 전 저장소 소유자에게 확인해야 합니다.
